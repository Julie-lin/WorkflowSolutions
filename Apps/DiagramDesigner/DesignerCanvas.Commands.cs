...
namespace DiagramDesigner
{
    ...
    public partial class DesignerCanvas
    {
        ...
        public static RoutedCommand ExcuteWorkflow = new RoutedCommand();
        public static RoutedCommand SaveRawFileList = new RoutedCommand();
        public static RoutedCommand OpenResultFile = new RoutedCommand();
        public static RoutedCommand ExcuteSampleWorkflow = new RoutedCommand();
        public static RoutedCommand ExcuteSelectedSamples = new RoutedCommand();
        ...
        public DesignerCanvas()
        {
            ...
            this.CommandBindings.Add(new CommandBinding(DesignerCanvas.ExcuteWorkflow, ExcuteWorkflow_Executed));
            this.CommandBindings.Add(new CommandBinding(DesignerCanvas.SaveRawFileList, SaveRawFileList_Executed));
            this.CommandBindings.Add(new CommandBinding(DesignerCanvas.OpenResultFile, OpenResultFile_Executed));
            this.CommandBindings.Add(new CommandBinding(DesignerCanvas.ExcuteSampleWorkflow, SamplesExcution_Executed));

            this.CommandBindings.Add(new CommandBinding(DesignerCanvas.ExcuteSelectedSamples, SelectedSamplesExcution_Executed, SelectedSamples_Enabled));
        ...
        private void New_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            //_batchsetupCanvas.ClearComponent();
        }
        ...
        private ComponentNode GetItemComponentParam(List<ComponentNode> parameters, Guid id)
        {
            return parameters.FirstOrDefault(p => p.Id==id);
        }
        private void Open_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            if (this.SelectionService.CurrentSelectedPage == TabPageName.CanvasPage)
            {
                OpenDesignView();
            }
            else if (this.SelectionService.CurrentSelectedPage == TabPageName.BatchSetupPage)
            {
                string file = GetOpenFile("Designer", "xml");
                if (string.IsNullOrEmpty(file))
                    return;
                SelectionService.RawFileViewModel.WorkflowFile = file;
                SelectionService.RawFileViewModel.DesignerItems = null;
                //OpenbatchSetupWorkflow();
                // OpenDesignView();
        ...
        private void OpenResultView()
        {
            try
            {
                //string file = GetOpenFile("Review", "xml");
                //PmfResult results = (PmfResult)XmlSerialization.OpenXml(file, typeof(ExecutionResult), ComponentConstants.AppExtraData);

                //this.selectionService.ResultViewModel.ResultInfo = results;
                //this.selectionService.ResultViewModel.RawFile = results.RawfileName;
                //List<UISearchResult> list = results.SearchResult;
                //foreach (UISearchResult s in list)
                //{
                //    UISearchResult uiResult = new UISearchResult(s);
                //    this.selectionService.ResultViewModel.Results.Add(uiResult);
                //}
            }
            catch (Exception)
            {
                //silent the threw as user may click on the wrong xml file
            }
        }

        //private ComponentCanvas _batchsetupCanvas;
        public void SetComponentCanvas(ComponentCanvas cavas)
        {
            //_batchsetupCanvas = cavas;
        }
        public void SetSampleSetupViewCanvas(ComponentCanvas cavas)
        {
            //_samplesetupCanvas = cavas;
        }

        private void OpenDesignView()
        {
            string file = GetOpenFile("Designer", "xml");
            if (string.IsNullOrEmpty(file))
                return;
            ...
            List<ComponentNode> list = XmlSerialization.OpenParameter(file, ProcessRunTimeLocator.GetAppExtraDataTypes());
            //ReverseUIFromComponent(list);
            List<DesignerItem> designItems = new List<DesignerItem>();
            foreach (XElement itemXML in itemsXML)
            {
                Guid id = new Guid(itemXML.Element("ID").Value);
                DesignerItem item = DeserializeDesignerItem(itemXML, id, 0, 0);
                var tb = item.Content as TextBox;
                item.OriginalBrush = tb.Background;
                item.ThisComponent = GetItemComponentParam(list, id);
                if (item.ThisComponent is IRecallFixeup)
                {
                    ((IRecallFixeup)item.ThisComponent).FixupCollectionItems();
                }
            ...
            this.SelectionService.RawFileViewModel.DesignerItems = designItems;
            string comFile = Path.GetDirectoryName(file) + "\\" + Path.GetFileNameWithoutExtension(file)  + "_comp" + ".xml";
            this.SelectionService.RawFileViewModel.WorkflowFile = comFile;
            this.SelectionService.SampleSetupViewModel.DesignerItems = designItems;
            OpenBatchSetupCanvas(itemsXML, connectionsXML, list);
            OpenSampleSetupCanvas(itemsXML, connectionsXML, list);
        }

        private void OpenbatchSetupWorkflow()
        {
            string file = GetOpenFile("BatchsetupView", "xml");
            if (string.IsNullOrEmpty(file))
                return;
            XElement root = XElement.Load(file);
            ...
            this.selectionService.RawFileViewModel.ClearBatchSetupCanvas();
            this.SelectionService.RawFileViewModel.DesignerItems = designItems;
            
            List<Connection> connections = new List<Connection>();
           // IEnumerable<XElement> connectionsXML = root.Elements("Connections").Elements("Connection");
            //this.SelectionService.RawFileViewModel.AddConnectionToBatchSetupView(connectionsXML);
            //this.SelectionService.RawFileViewModel.FireWorkflowChanged(designItems, connections, _batchsetupCanvas);
            
        }

        private void OpenBatchSetupCanvas(IEnumerable<XElement> itemsXML, IEnumerable<XElement> connectionsXML, List<ComponentNode> list)
        {
            //_batchsetupCanvas.ClearComponent();
            //foreach (XElement itemXML in itemsXML)
            //{
            ...
            //    _batchsetupCanvas.AddToChildren(item);
            //}
            ...
            //    Connection connection = new Connection(sourceConnector, sinkConnector);
            //    Canvas.SetZIndex(connection, Int32.Parse(connectionXML.Element("zIndex").Value));
            //    _batchsetupCanvas.AddToConnectionChildren(connection);
            //}
            //_batchsetupCanvas.InvalidateVisual();
            //_batchsetupCanvas.SetComponentNode(itemsXML);
            //_batchsetupCanvas.InvalidateVisual();
            //_batchsetupCanvas.SetConnectionNode(connectionsXML);
        }
        private void OpenSampleSetupCanvas(IEnumerable<XElement> itemsXML, IEnumerable<XElement> connectionsXML, List<ComponentNode> list)
        {
            //_samplesetupCanvas.ClearComponent();
            foreach (XElement itemXML in itemsXML)
            {
                Guid id = new Guid(itemXML.Element("ID").Value);
                DesignerItem item = DeserializeDesignerItem(itemXML, id, 0, 0);
                ...
                //_samplesetupCanvas.AddToChildren(item);
            }
            ...
            foreach (XElement connectionXML in connectionsXML)
            {
                ...
                Connection connection = new Connection(sourceConnector, sinkConnector);
                Canvas.SetZIndex(connection, Int32.Parse(connectionXML.Element("zIndex").Value));
                //_samplesetupCanvas.AddToConnectionChildren(connection);
            }
        }

        void ReverseUIFromComponent(List<ComponentNode> list)
        {
            foreach (var comp in list)
            {
                DesignerItem item = new DesignerItem(comp.Id);

                ComponentUIInfo itemUIInfo = ((IGetComponentUI)comp).ComponentUi;
                if (itemUIInfo == null)
                    break;

                //DesignerItem item = new DesignerItem(id);
                item.Width = itemUIInfo.Width;
                item.Height = itemUIInfo.Heigh;
                item.ParentID = itemUIInfo.ParentId;
                item.IsGroup = itemUIInfo.IsGroup;
                Canvas.SetLeft(item, itemUIInfo.Left + 0);
                Canvas.SetTop(item, itemUIInfo.Top + 0);
                Canvas.SetZIndex(item, itemUIInfo.ZIndex);


                object content = XamlReader.Load(XmlReader.Create(new StringReader(itemUIInfo.Content)));
                item.Content = content;

                item.ThisComponent = comp;
                this.Children.Add(item);
                SetConnectorDecoratorTemplate(item);
            }
        }
        
        void OpenComponentResultFile()
        {
            //string file = GetOpenFile("Component", "xml");
            //if (file.Contains(AppConstants.PmfViewResult ) == true)
            //{
            //    PmfResult result = (PmfResult)XmlSerialization.OpenXml(file, typeof(PmfResult), null);
            //    this.selectionService.ResultViewModel.RawFile = result.RawfileName;
            //    foreach (SearchResult s in result.SearchResult)
            //    {
            //        UISearchResult uiResult = new UISearchResult(s);
            //        this.selectionService.ResultViewModel.Results.Add(uiResult);
            //    }
        ...
        //string rawFile = @"C:\Prototype\WorkflowTest\TestData\bsa_stdTOP5_.RAW";
        //string file = @"C:\Prototype\WorkflowTest\TestData\iman.xml";
        ...
        private void Save_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            if (this.SelectionService.CurrentSelectedPage == TabPageName.CanvasPage)
            {
                string file = GetSaveFile("xml");
                if (!string.IsNullOrEmpty(file))
                {
                    IEnumerable<DesignerItem> designerItems = this.Children.OfType<DesignerItem>();
                    IEnumerable<Connection> connections = this.Children.OfType<Connection>();
                    SetComponentUIElementsItems(designerItems);
                    XElement designerItemsXML = SerializeDesignerItems(designerItems);
                    XElement connectionsXML = SerializeConnections(connections);


                    var starts = designerItems.ToList().FindAll(c => c.ThisComponent.StartNode == true);
                    foreach (var start in starts)
                    {
                        if (start.ThisComponent.ParentIdList.Count > 0)
                        {
                            start.ThisComponent.StartNode = false;
                        }
                    }
 
                    var startup = designerItems.FirstOrDefault(c => c.ThisComponent.StartNode == true);
                    if (startup != null)
                    {
                        SetTreeConnectionInfo(startup.ThisComponent, connections);
                    }
                    if (((IStartUpNode) startup.ThisComponent).ExecuteWorkflowId == null)
                    {
                        ((IStartUpNode) startup.ThisComponent).ExecuteWorkflowId = Guid.NewGuid();
                    }
                    ...
                    root.Save(file);
                    //SaveFile(root);
                    string path = Path.GetDirectoryName(file);//.GetFullPath(file);
                    string name = Path.GetFileNameWithoutExtension(file);
                    string compFile = path + "\\" + name + "_comp" + ".xml";
                    SaveParameters(designerItems, connections, compFile);
                    this.SelectionService.RawFileViewModel.DesignerItems = designerItems.ToList();
                    this.SelectionService.RawFileViewModel.WorkflowFile = compFile;
                }
                
            }
        }

        private void SaveRawFiles()
        {
            string file = GetSaveFile("xml");
            //SaveFile(root);
            string path = Path.GetDirectoryName(file);//.GetFullPath(file);
            string name = Path.GetFileNameWithoutExtension(file);
            string compFile = path + "\\" + name + "_raw" + ".xml";

        }

        private void SaveParameters(IEnumerable<DesignerItem> items, IEnumerable<Connection>  connections, string file)
        {
            var list = GetDesignItemComponentInfo(items, connections);
            XmlSerialization.SaveDerivedObjectToXML(file, list, typeof(List<ComponentNode>), ProcessRunTimeLocator.GetAppExtraDataTypes());
        }

        private static List<ComponentNode> GetDesignItemComponentInfo(IEnumerable<DesignerItem> items, IEnumerable<Connection> connections)
        {
            List<ComponentNode> list = new List<ComponentNode>();

            foreach (var item in items.ToList())
            {
                item.ThisComponent.ParentIdList.Clear();
                
                item.ThisComponent.Id = item.ID;
                var enumerator = connections.GetEnumerator();
                while (enumerator.MoveNext())
                {
                    if (enumerator.Current == null)
                    {
                        list.Add(item.ThisComponent); //if no connections still add item
                    }
                    else
                    {
                        if (enumerator.Current.Sink.ParentDesignerItem.ID.Equals(item.ID))
                            {
                                item.ThisComponent.ParentIdList.Add(enumerator.Current.Source.ParentDesignerItem.ID);
                            }
                    }
                }
                list.Add(item.ThisComponent);
            }
            return list;
        }
        ...
        #region Excute workflow command
        MnemeBatch batch = new MnemeBatch();
        private void ExcuteWorkflow_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            bool testNewIdea = false;
            if (string.Compare(System.Configuration.ConfigurationSettings.AppSettings["TestNewIdea"], "true", true) == 0)
            {
                testNewIdea = true;
            }

            //BatchNameWindow batchNameWindow = new BatchNameWindow();
            
            //if (batchNameWindow.ShowDialog() != true) return;

            //string submitBatchName = batchNameWindow.Name;
            //string submitDescription = batchNameWindow.Description;
            
            if (this.SelectionService.CurrentSelectedPage == TabPageName.BatchSetupPage)
            {
                List<ComponentNode> list = selectionService.RawFileViewModel.GetComponents();
                if (list.Count <= 1)
                {
                    MessageBox.Show("please select a component workflow");
                    return;
                }
                batch = (MnemeBatch)selectionService.RawFileViewModel.GetProcessBatch();
                if (batch.Groups.Count == 0 || batch.Groups[0].Jobs.Count == 0)
                {
                    MessageBox.Show("Please select a measurement");
                }
                batch.Name = "default batch";
                batch.Description = "no description";
                batch.EfConnectionString = selectionService.RawFileViewModel.GetEfConnectionString();
                batch.AdoConnectionString = selectionService.RawFileViewModel.GetAdoConnectionString();

                //string path = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
                string machineName = selectionService.RawFileViewModel.GetDbMachineName();
                if (string.IsNullOrEmpty(machineName)) return;
                if (testNewIdea)
                {
                    SubmitCompositeWorkflowToServer(batch, list, machineName);
                }
                else
                {
                    SubmitToServer(batch, list);
                }
                selectionService.RawFileViewModel.SetUIProcessBatchRunningStatus();

            }
            else if (this.SelectionService.CurrentSelectedPage == TabPageName.SampleSetupPage)
            {
                List<ComponentNode> list = selectionService.SampleSetupViewModel.GetComponents();
                ProcessBatch batch = selectionService.SampleSetupViewModel.GetProcessBatch();
                Random random = new Random();
                int count = random.Next();

                //assign a execution tag
                ComponentNode startup = list.FirstOrDefault(c => c.StartNode == true);
                if (startup != null && list.Count > 1)
                {
                    //startup.TreeExecutionTag = startup.ComponentName + DateTime.Now.ToShortTimeString();
                    //Utility.CallServer(
                    //    proxy => proxy.ExcuteTracebleACQComponentActivity(batch, list, "abc" + count.ToString()));
        ...
        private static void SubmitToServer(ProcessBatch batch, List<ComponentNode> list)
        {
            Random random = new Random();
            int count = random.Next();
            ...
            {
                //ComponentNode startup = list.FirstOrDefault(c => c.StartNode == true);
                //startup.TreeExecutionTag = startup.ComponentName;// +DateTime.Now.t;
                //Utility.CallServer(proxy => proxy.ExcuteTracebleComponentActivity(batch, "abv" + count, list));
            }
        }
        private static void SubmitCompositeWorkflowToServer(ProcessBatch batch, List<ComponentNode> list, string machineName) 
        {
            if (list.Count > 1)
            {
                try
                {
                    ...
                    ComponentNode startup = list.FirstOrDefault(c => c.StartNode == true);
                    startup.TreeExecutionTag = startup.ComponentName;// +DateTime.Now.t;
                    IProcessingService service = ChannelFactoryUtility.CreateProcessingEngineService(machineName);
                    service.ExecuteCompositeBatchSequentialActivity(batch, list, Guid.NewGuid());
                    //ChannelFactoryUtility.CallProcessServer(proxy => proxy.ExecuteCompositeBatchSequentialActivity(batch, list,
                    //    Guid.NewGuid()));
        ...
        private void SamplesExcution_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            string file = GetOpenFile("sample", "xml");
            List<ComponentNode> list = XmlSerialization.OpenParameter(file, ProcessRunTimeLocator.GetAppExtraDataTypes());

            List<ClientSample> samples = new List<ClientSample>();
            samples.AddRange(selectionService.SampleSetupViewModel.Samples);
            ProcessBatch batch = CreateSampleProcessBatch(samples);
            ...
            if (list.Count > 1)
            {
                //startup.TreeExecutionTag = startup.ComponentName;// +DateTime.Now;
                //Utility.CallServer(
                //    proxy => proxy.ExcuteTracebleACQComponentActivity(batch, list, "abc" + count.ToString()));
            ...
            //rawFiles.AddRange(selectionService.ViewModel.RawFiles);
           
            //if (list.Count > 1)
            //{
            //    Utility.CallServer(proxy => proxy.ExcuteCompositeActivity(list, rawFiles));
                
            //}
        }
        private void SelectedSamplesExcution_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            string file = GetOpenFile("sample", "xml");
            List<ComponentNode> list = XmlSerialization.OpenParameter(file, ProcessRunTimeLocator.GetAppExtraDataTypes());
        ...
        private void SelectedSamples_Enabled(object sender, CanExecuteRoutedEventArgs e)
        {
            e.CanExecute = true;
        }


        ProcessBatch CreateSampleProcessBatch(List<ClientSample> samples)
        {
           ...
           // batch.Path = @"C:\Prototype\ProcessWorflowTestData\Batches\";

            string rawFilePath = @"C:\SimSourceData";
            var jobs = CreateTestData.CreateSampleJobs(rawFilePath, samples);
        ...
        private void OpenResultFile_Executed(object sender, ExecutedRoutedEventArgs e)
        {
        }

        private void SaveRawFileList_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            throw new NotImplementedException();
        }

        void CreateTestResultData()
        {
            //this.selectionService.ResultViewModel.Results.Add(new UISearchResult(2.0, "first ", 300.22, new LightningPeak()));
            //this.selectionService.ResultViewModel.Results.Add(new UISearchResult(3.0, "2 ", 310.22, new LightningPeak()));
            //this.selectionService.ResultViewModel.Results.Add(new UISearchResult(4.0, "3 ", 320.22, new LightningPeak()));
            //this.selectionService.ResultViewModel.Results.Add(new UISearchResult(5.0, "4 ", 330.22, new LightningPeak()));
        }
        ...
        private string GetOpenFile(string view, string ext)
        {
            OpenFileDialog openFile = new OpenFileDialog();
            string filter = string.Format("{0} Files (*.{1})|*.{2}|All Files (*.*)|*.*", view, ext, ext);
        ...
        private string GetSaveFile(string ext)
        {
            SaveFileDialog saveFile = new SaveFileDialog();
            string filter = String.Format("Files (*.{0})|*.{1}|All Files (*.*)|*.*", ext, ext);
        ...
        private XElement SerializeDesignerItems(IEnumerable<DesignerItem> designerItems)
        {
            ...
            XElement serializedItems = new XElement("DesignerItems",
                                       ...
                                       select new XElement("DesignerItem",
                                                  ...
                                                  new XElement("ThisComponent", item.ThisComponent)
                                                  //new XElement("OriginalColor", item.OriginalColor.ToString())
                                              )
                                   );
        ...
        private void SetComponentUIElementsItems(IEnumerable<DesignerItem> designerItems)
        {
            var inumerator = designerItems.GetEnumerator();
            while (inumerator.MoveNext())
            {
                var current = (DesignerItem) inumerator.Current;
                if (current.ThisComponent is IGetComponentUI)
                {
                    var compUI = ((IGetComponentUI) ((current).ThisComponent)).ComponentUi;

                    compUI.Left = GetLeft(current);
                    compUI.Top = GetTop(current);
                    compUI.Width = current.Width;
                    compUI.Heigh = current.Height;
                    compUI.ZIndex = Canvas.GetZIndex(current);
                    compUI.IsGroup = current.IsGroup;
                    compUI.ParentId = current.ParentID;
                    var contentXaml = XamlWriter.Save((current).Content);
                    compUI.Content = contentXaml;
                }
            }
        }

        private void SetTreeConnectionInfo(ComponentNode startup, IEnumerable<Connection> connections)
        {
            var treeInfo = ((IGetComponentConnectionInfo)startup).TreeConnectionInfo;
            treeInfo.Clear();
            var inumerator = connections.GetEnumerator();
            while (inumerator.MoveNext())
            {
                var current = (Connection)inumerator.Current;
                
                ComponentConnectionInfo conn = new ComponentConnectionInfo();
                conn.SourceId = current.Source.ParentDesignerItem.ID;
                conn.SinkId = current.Sink.ParentDesignerItem.ID;
                conn.SourceConnectorName = current.Source.Name;
                conn.SinkConnectorName = current.Sink.Name;
                conn.SinkArrowSymbol = current.sinkArrowSymbol.ToString();
                conn.ZIndex = Canvas.GetZIndex(current);
                treeInfo.Add(conn);
            }
        ...
        private static DesignerItem DeserializeDesignerItem(XElement itemXML, Guid id, double OffsetX, double OffsetY)
        {
            ...
            item.Content = content;
           // item.OriginalColor =  System.Windows.Media.Brushs
            return item;
        }
        ...
        private void CopyCurrentSelection()
        {
            ...
            //SetComponentUIElementsItems(selectedDesignerItems);
